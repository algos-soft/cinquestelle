 ...
 * <li> Incolla il testo dagli appunti </li>
 ...
 * @author gac
 * @version 1.0    / 7-gen-2005 ore 9.46.00
 */
public final class AzIncollaTesto extends AzAdapterAction {

    /**
     * Testo della chiave per recuperare l'azione (obbligatorio)
     */
    private static final String CHIAVE = Azione.INCOLLA_TESTO;

    /**
     * Testo di default da mostrare
     * (obbligatorio per le azioni dei menu/bottoni/toolbar)
     */
    private static final String NOME = Testo.get(CostanteTesto.INCOLLA_TESTO);

    /**
     * Nome del disegno di default da mostrare
     * (consigliato per le azioni dei menu/bottoni/toolbar)
     */
    private static final String ICONA_PICCOLA = "Paste16";

    private static final String ICONA_MEDIA = "Paste24";

    private static final String ICONA_GRANDE = "Paste";

    /**
     * Descrizione di default per il tooltiptext (facoltativo)
     */
    private static final String TESTO_TIP = Testo.get(CostanteTesto.INCOLLA_TESTO_TIP);

    /**
     * Descrizione di default per l'aiuto in linea (facoltativo)
     */
    private static final String TESTO_HELP = Testo.get(CostanteTesto.INCOLLA_TESTO_HELP);

    /**
     * Carattere del tasto acceleratore (facoltativo)
     */
    private static final char ACCELERATORE = 'V';
...
//            super.getGestore().incollaTesto(unEvento, this);
        } catch (Exception unErrore) { // intercetta l'errore
            Errore.crea(unErrore);
        }// fine del blocco try-catch
    }
