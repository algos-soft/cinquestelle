 ...
 * <li> Copia il testo selezionato </li>
 ...
 * @author gac
 * @version 1.0    / 7-gen-2005 ore 9.43.30
 */
public final class AzCopiaTesto extends AzAdapterAction {

    /**
     * Testo della chiave per recuperare l'azione (obbligatorio)
     */
    private static final String CHIAVE = Azione.COPIA_TESTO;

    /**
     * Testo di default da mostrare
     * (obbligatorio per le azioni dei menu/bottoni/toolbar)
     */
    private static final String NOME = Testo.get(CostanteTesto.COPIA_TESTO);

    /**
     * Nome del disegno di default da mostrare
     * (consigliato per le azioni dei menu/bottoni/toolbar)
     */
    private static final String ICONA_PICCOLA = "Copy16";

    private static final String ICONA_MEDIA = "Copy24";

    private static final String ICONA_GRANDE = "Copy";

    /**
     * Descrizione di default per il tooltiptext (facoltativo)
     */
    private static final String TESTO_TIP = Testo.get(CostanteTesto.COPIA_TESTO_TIP);

    /**
     * Descrizione di default per l'aiuto in linea (facoltativo)
     */
    private static final String TESTO_HELP = Testo.get(CostanteTesto.COPIA_TESTO_HELP);

    /**
     * Carattere del tasto acceleratore (facoltativo)
     */
    private static final char ACCELERATORE = 'C';
...
//            super.getGestore().copiaTesto(unEvento, this);
        } catch (Exception unErrore) { // intercetta l'errore
            Errore.crea(unErrore);
        }// fine del blocco try-catch
    }
