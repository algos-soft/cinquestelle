 ...
 * <li> Annulla l'ultima operazione eseguita (Undo) </li>
 ...
 * @author gac
 * @version 1.0    / 7-gen-2005 ore 9.32.47
 */
public final class AzAnnullaOperazione extends AzAdapterAction {

    /**
     * Testo della chiave per recuperare l'azione (obbligatorio)
     */
    private static final String CHIAVE = Azione.ANNULLA_OPERAZIONE;

    /**
     * Testo di default da mostrare
     * (obbligatorio per le azioni dei menu/bottoni/toolbar)
     */
    private static final String NOME = Testo.get(CostanteTesto.ANNULLA_OPERAZIONE);

    /**
     * Nome del disegno di default da mostrare
     * (consigliato per le azioni dei menu/bottoni/toolbar)
     */
    private static final String ICONA_PICCOLA = "Undo16";

    private static final String ICONA_MEDIA = "Undo24";

    private static final String ICONA_GRANDE = "Undo";

    /**
     * Descrizione di default per il tooltiptext (facoltativo)
     */
    private static final String TESTO_TIP = Testo.get(CostanteTesto.ANNULLA_OPERAZIONE_TIP);

    /**
     * Descrizione di default per l'aiuto in linea (facoltativo)
     */
    private static final String TESTO_HELP = Testo.get(CostanteTesto.ANNULLA_OPERAZIONE_HELP);

    /**
     * Carattere del tasto acceleratore (facoltativo)
     */
    private static final char ACCELERATORE = 'Z';
...
//            super.getGestore().annullaOperazione(unEvento, this);
        } catch (Exception unErrore) { // intercetta l'errore
            Errore.crea(unErrore);
        }// fine del blocco try-catch
    }
