 ...
 * <li> Apre la scheda con un nuovo record </li>
 ...
 */
public class AzNuovoRecord extends AzAdapterAction {

    /**
     * Testo della chiave per recuperare l'azione (obbligatorio)
     */
    private static final String CHIAVE = Azione.NUOVO_RECORD;

    /**
     * Testo di default da mostrare (obbligatorio)
     */
    private static final String NOME = Testo.get(CostanteTesto.NUOVO_RECORD);

    /**
     * Nome del disegno di default da mostrare (consigliato)
     */
    private static final String ICONA_PICCOLA = "Nuovo16";

    private static final String ICONA_MEDIA = "Nuovo24";

    private static final String ICONA_GRANDE = "Nuovo";

    /**
     * Descrizione di default per il tooltiptext (facoltativo)
     */
    private static final String TIP = Testo.get(CostanteTesto.NUOVO_RECORD_TIP);

    /**
     * Descrizione di default per l'aiuto in linea (facoltativo)
     */
    private static final String HELP = Testo.get(CostanteTesto.NUOVO_RECORD_HELP);
    ...
    public void actionPerformed(ActionEvent unEvento) {
        try { // prova ad eseguire il codice
            ...
            super.getGestore().nuovoRecord(unEvento, this);
        } catch (Exception unErrore) { // intercetta l'errore
            Errore.crea(unErrore);
        }// fine del blocco try-catch
    }
