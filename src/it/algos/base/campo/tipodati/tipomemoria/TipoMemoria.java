/**
 * Title:        TipoMemoria.java
 * Package:      it.algos.base.campo.tipomemoria
 * Description:  Interfaccia comune ai Tipi Dati Memoria
 * Copyright:    Copyright (c) 2003
 * Company:      Algos s.r.l.
 * @author Guido Andrea Ceresa & Alessandro Valbonesi  /  alex
 * @version 1.0  /
 * Creato:       il 16 ottobre 2003 alle 12.39
 */
//-----------------------------------------------------------------------------
// Copyright (c) 2003  Algos s.r.l. All Rights Reserved.
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
//  Package di questa interfaccia e classi importate
//-----------------------------------------------------------------------------
package it.algos.base.campo.tipodati.tipomemoria;

import it.algos.base.campo.base.Campo;
import it.algos.base.campo.tipodati.TipoDati;

/**
 * Questa interfaccia e' responsabile di: <br>
 * A - Presentare all'esterno le classi che descrivono
 * i vari tipi dati Memoria
 *
 * @author Guido Andrea Ceresa & Alessandro Valbonesi  /  alex
 * @version 1.0  /  16 ottobre 2003 ore 16.39
 */
public interface TipoMemoria extends TipoDati {

    /**
     * Controlla se un campo e' modificato
     */
    public abstract boolean isCampoModificato(Campo unCampo);


}// fine della interfaccia

