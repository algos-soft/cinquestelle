 ...
 */
public final class ContoPrintable extends StampabileNumPagina {
	...
	/**
	 * Mappa della prima riga di addebito per ogni pagina
	 * key - il numero di pagina
	 * value - l'indice dell'ultima riga stampata sulla pagina
	 */
	private HashMap<Integer, Integer> mappaRighePerPagina = new HashMap<Integer, Integer>();
	
	// il massimo numero di pagina (1 per la prima ecc...)
	private int maxNumPagina=-1;
	
	// numero della pagina correntemente in stampa
	private int numPagCorrente;
	 ...
	 */
	public ContoPrintable(int codice, Conto.TipiStampa tipo, boolean raggruppa) {
	...
	private void inizia() {
    	...
    	// simula la stampa per calcolare il numero di pagine totali
		simulatePrint();
    ...
    public int print(Graphics g, PageFormat pf, int pageIndex) {
    	...
    	// memorizza il numero di pagina correntemente in stampa
		this.numPagCorrente = pageIndex+1;

    	super.print(g, pf, pageIndex);
    	
		if ((maxNumPagina >=0) && (pageIndex+1>maxNumPagina)) {
			return NO_SUCH_PAGE;
		}else{	
			return PAGE_EXISTS;
		}
    ...
    /**
     * Simula una stampa per determinare e registrare il numero totale di pagine
     */
	private void simulatePrint(){
		
		// oggetto Graphics2D fittizio
		...
		// prova a stampare - il metodo stampaAddebiti 
		// regola la variabile di istanza maxNumPagina
		boolean fine = false;
		while (!fine) {
			int codUscita = print(g2, pf, pageIndex);
			if (codUscita==NO_SUCH_PAGE) {
				fine=true;
			}
			pageIndex++;
		}
	 ...
	 * - disegnare la pagina richiesta nell'oggetto grafico che viene passato.
	 ...
	 * (cioè se è stata richiesta una pagina inesistente)
	 */
	public boolean printPagina(Graphics2D pagina, PageFormat formato, int numPagina) {
		...
		try { // prova ad eseguire il codice
			...
			/* carica i dati per la stampa (e li mette in cache) */
			...
			/*
			 * prima di cominciare a stampare la pagina resetta la posizione della penna
			 */
			yPenna = 0;
			...
			/* stampa gli addebiti */
			addebitiEsauriti = this.stampaAddebiti(pagina, numPagina);
	 ...
	 */
	private boolean stampaAddebiti(Graphics2D pagina, int numPagina) {
		/* variabili e costanti locali di lavoro */
		...
		try { // prova ad eseguire il codice
			...
			// determina il numero della prima riga da stampare
			if (numPagina == 1) { // pagina 1
				riga = 0;
			}
			else { // pagine dalla 2 in poi
				riga = mappaRighePerPagina.get(numPagina - 1) + 1;
			}

			// regola il progressivo iniziale per la pagina
			progr = getProgressivoIniziale(numPagina);

			/* ciclo di stampa addebiti */
			while (!fine) {

				/*
				 * controlla che non sia esaurito l'elenco degli addebiti da stampare
				 */
				continua = true;
				if (continua) {
...
//						// la prima volta che raggiunge la fine degli addebiti imposta
//						// il max num pagina e poi non lo tocca più
//						if (maxNumPagina==-1) {
//							maxNumPagina=numPagina;
//						}
				 ...
				 * controlla che la penna non sia avanzata oltre la fine dell'area stampabile
				 */
				if (continua) {
			...
			// alla fine della stampa righe mette l'indice
			// dell'ultima riga stampata nella mappa
			if (!(mappaRighePerPagina.containsKey(numPagina))) {
				mappaRighePerPagina.put(numPagina, 0);
			}
			mappaRighePerPagina.put(numPagina, riga - 1);
	...
	/**
	 * Ritorna l'importo progressivo iniziale per una data pagina.
	 * @param numPagina il numero di pagina
	 * @return il progressivo iniziale per la pagina
	 */
	private double getProgressivoIniziale(int numPagina) {
		double progr=0;
		if (numPagina > 1) {
			int ultRigaPagPrec = mappaRighePerPagina.get(numPagina-1);
			Dati addebiti = getAddebiti();
			
			for(int riga=0; riga<=ultRigaPagPrec; riga++){
                int quantita = addebiti.getIntAt(riga, campoAddQuantita);
                double prezzo = addebiti.getDoubleAt(riga, campoAddPrezzo);
	...
	private boolean stampaRiepilogo() {
		...
		try { // prova ad eseguire il codice
			...
			/*
			 * esegue una stampa simulata per vedere se ci sta nella pagina
			 */
			this.stampaRiepilogo(false);
			hRiepilogo = yPenna - oldyPenna;

			/*
			 * se la penna non e andata oltre al margine inferiore, ripristina la posizione della penna, esegue
			 * effettivamente la stampa e ritorna true
			 */
			if (yPenna <= this.getyMax()) {
	...
	private void stampaRiepilogo(boolean flag) {
		...
		try { // prova ad eseguire il codice
			...
			// dopo aver stampato il riepilogo imposta
			// il max num pagina e poi non lo tocca più
			if (flag) {
				if (maxNumPagina==-1) {
					maxNumPagina=numPagCorrente;
				}
			}
	...
	private void stampaRiepilogoTotali(boolean flag) {
		...
		try { // prova ad eseguire il codice
			...
			/*
			 * determina il margine sinistro per ottenere l'allineamento a destra del riepilogo
			 */
	...
	private void caricaDatiIntestazione() {
		...
		try { // prova ad eseguire il codice
			...
			/* data di arrivo */
			this.dataArrivo = (Date) modConto.query().valoreCampo(Conto.Cam.validoDal.get(), this.getCodConto());
	...
	private void raggruppaPensioni() {
		...
		try { // prova ad eseguire il codice
			...
			/*
			 * crea un nuovo oggetto dati per contenere i dati raggruppati
			 */
			...
			/*
			 * recupera l'oggetto dati corrente, lo spazzola e lo copia nel nuovo oggetto dati effettuando il
			 * raggruppamento
			 */
			...
			for (int k = 0; k < dati.getRowCount(); k++) {
				...
				/*
				 * cambio della data addebito - riporta il totale accumulato sulla precedente riga di pensione - crea
				 * una nuova riga di pensione
				 */
				if (!data.equals(oldData)) {
					...
					dati2.setValueAt(riga, campoListinoAmbito, Listino.AmbitoPrezzo.pensioniComplete.getCodice());
					ultRigaPens = riga;
				...
				/*
				 * se riga extra, copia la riga nei dati nuovi se riga pensione, accumula il totale
				 */
	...
	private void caricaDatiRiepilogo() {
		...
		try { // prova ad eseguire il codice
			...
			for (int k = 0; k < addebiti.getRowCount(); k++) {
				...
				/*
				 * attribuisce l'importo al totale pensione o extra a seconda dell'ambito prezzo della voce di listino
				 */
    ...
    protected int getNumeroPagina() {
        return numPagCorrente;
    }
    
    public int getQuantePagine(){
    	return maxNumPagina;
    }
