...
class TbBarMesi extends TbBar {
     ...
     */
    public TbBarMesi(PanGrafi panGrafi) {
    ...
    protected void createModelAndView() {
        ...
        try { // prova ad eseguire il codice
            ...
            GraphLayoutCache view = new GraphLayoutCache(model, new MeseViewFactory());
            this.setModel(model);
     ...
     * @param giorno data alla quale iniziare
     * @param quantiGG lunghezza in giorni della cella
     */
    private void addMese(Date giorno, int quantiGG) {

        try {    // prova ad eseguire il codice

            /* aggiunge al grafo e alla lista */
            int pos = this.getPos(giorno);
            int num= Lib.Data.getNumeroMese(giorno);
            String nome = Lib.Data.getNomeMese(num);
            GraphCell cell = new CellMese(nome,pos, quantiGG);
    ...
    void popola() {
        ...
        try {    // prova ad eseguire il codice
            ...
            Date data=d1;
            int old_mese= Lib.Data.getNumeroMese(data);
            int quantiGG=0;
            Date dataStart=data;
            while (Lib.Data.isPrecedenteUguale(d2, data)) {
                int mese = Lib.Data.getNumeroMese(data);
                if (mese!=old_mese) {
                    this.addMese(dataStart, quantiGG);
                    old_mese=mese;
                    quantiGG=0;
                    dataStart=data;
                }// fine del blocco if
    ...
    /**
     * ViewFactory per le celle di tipo Mese.
     * </p>
     */
    private class MeseViewFactory extends DefaultCellViewFactory {

        protected VertexView createVertexView(Object cell) {
            return new CellViewMese(cell);
        }
